const STORE='ing_pen_v4';
const OLD_STORES=['ing_pen_v3','ingresos_pension_public_v1'];
...
function load(){try{let cur=localStorage.getItem(STORE);if(cur)return JSON.parse(cur)||{records:[],pensions:[]};for(let k of OLD_STORES){let old=localStorage.getItem(k);if(old)return JSON.parse(old)||{records:[],pensions:[]}}return{records:[],pensions:[]}}catch(e){return{records:[],pensions:[]}}}
...
function hasWord(s,w){return nk(s).includes(nk(w))}
...
function years(){let a=[new Date().getFullYear(),...S.records.map(r=>r.paymentYear||y(r.paymentDate)),...S.records.map(r=>r.managementYear),...S.pensions.map(p=>y(p.paymentDate))].filter(Boolean);let u=[...new Set(a)].sort((a,b)=>b-a),cur=$('year').value||u[0];$('year').innerHTML=u.map(v=>`<option ${v==cur?'selected':''}>${v}</option>`).join('')}
function render(){
  years();
  let Y=+$('year').value||new Date().getFullYear(),scope=$('scope').value;
  let R=S.records.filter(r=>(scope==='tax'?+r.managementYear:+(r.paymentYear||y(r.paymentDate)))===Y),P=S.pensions.filter(p=>y(p.paymentDate)===Y);
...
function tbl(a,del=false){if(!a.length)return'<p class="note">Sin ingresos.</p>';return'<table><tr><th>Fecha</th><th>Tipo</th><th>Detalle</th><th>Líquido</th><th>Base</th><th>Imp.</th><th></th></tr>'+a.slice().sort((a,b)=>String(b.paymentDate).localeCompare(String(a.paymentDate))).map(r=>`<tr><td>${r.paymentDate||''}</td><td>${typ(r.type)}<br><span class="note">${status(r)}</span></td><td>${r.description||''}<br><span class="note">Trib./gestión ${r.managementYear||''} · ${r.source||''}</span></td><td class="mono">${money(r.netClp)}</td><td class="mono">${money(r.taxableBaseClp)}</td><td class="mono">${money(r.withheldTaxClp)}</td><td>${del?`<button class="btn btn2" onclick="delRec('${r.id}')">Borrar</button>`:''}</td></tr>`).join('')+'</table>'}
...
function delRec(id){S.records=S.records.filter(r=>r.id!==id);save()}function delPen(id){S.pensions=S.pensions.filter(p=>p.id!==id);save()}
...
function addIncome(){let d=$('iDate').value||new Date().toISOString().slice(0,10),uf=dec($('iUf').value),net=clp($('iNet').value);upsertRecord({id:'r'+Date.now(),paymentDate:d,paymentYear:y(d),type:$('iType').value,managementYear:clp($('iMgmt').value)||y(d),description:$('iDesc').value||'Ingreso manual',netClp:net,taxableBaseClp:clp($('iBase').value),withheldTaxClp:clp($('iTax').value),ufValue:uf,netUf:uf?net/uf:0,source:'Manual',isProjected:false});save()}
...
async function readExcel(){
  ...
  try{
    ...
    let Y=clp($('xlsYear').value)||gyear(f.name);$('xlsYear').value=Y;xprev=[];
    rows.forEach((r,i)=>{
      ...
      let isProjected=isNextYear||label.includes('proyect');
      let type=isBonus?'bonus':'salary';
      let mgmt=isBonus||isNextYear?Y-1:Y;
      let d=payDate(Y,M),desc=isBonus?`Bono proyectado ${mn[M]} ${Y} · tributa ${mgmt}`:`Liquidación ${mn[M]} ${Y}`;
      xprev.push({id:'x'+Date.now()+i,paymentDate:d,paymentYear:Y,type,managementYear:mgmt,description:desc,grossClp:gross,deductionsClp:clp(pick(r,['Descuentos','Total Descuentos'])),taxableBaseClp:base||baseLiq,withheldTaxClp:tax,retencionSiiClp:clp(pick(r,['Retension SII','Retención SII','Retensión SII'])),netClp:liq||total,finalIncomeClp:total||liq,afpClp:clp(pick(r,['AFP',' AFP'])),healthClp:clp(pick(r,['Isapre','Salud'])),afcClp:clp(pick(r,['AFC'])),ccafClp:clp(pick(r,['CAFR','CCAF'])),ufValue:dec(pick(r,['UF','Valor UF','UF fecha pago'])),source:'Excel '+f.name,isProjected});
      Object.keys(r).filter(k=>nk(k).startsWith('ing extra')).forEach((k,j)=>{let a=clp(r[k]);if(a>0)xprev.push({id:'e'+Date.now()+i+j,paymentDate:d,paymentYear:Y,type:'extra',managementYear:Y,description:String(pick(r,['Motivo','Motivo_'+(j+1)])||`Ingreso extra ${mn[M]} ${Y}`),netClp:a,taxableBaseClp:0,withheldTaxClp:0,source:'Excel '+f.name,isProjected:false})});
    });
    $('tblPrev').innerHTML=tbl(xprev);$('prevX').classList.remove('hide');$('xlsMsg').textContent=`Leídos ${xprev.length} registros. Revisa y guarda. Filas vacías o meses proyectados sin haberes se ignoran.`;
...
function parseLiquid(text,source){
  ...
  return{id:'r'+Date.now(),paymentDate:d,paymentYear:+yy,type,managementYear:type==='bonus'?+yy-1:+yy,description:(type==='bonus'?'Bono real ':'Liquidación ')+mn[M]+' '+yy,grossClp:clp(tot[1]),deductionsClp:clp(tot[2]),taxableBaseClp:clp(imp[1]),withheldTaxClp:clp(imp[2]),netClp:grab(c,/Monto\s+l[ií]quido\D+([\d.,]+)/i),afpClp:grab(c,/AFP[^\n]*?([\d.,]+)\s*(?:\n|$)/i),healthClp:grab(c,/(VIDA\s+TRES|ISAPRE|SALUD)[^\n]*?([\d.,]+)\s*(?:\n|$)/i),afcClp:grab(c,/AFC\s+([\d.,]+)/i),ccafClp:grab(c,/PRESTAMO\s+CCAF[^\n]*?([\d.,]+)/i),source:'PDF '+source,isProjected:false};
}
async function readPdf(){let f=$('pdf').files[0];if(!f)return $('pdfMsg').textContent='Selecciona PDF.';try{pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';let doc=await pdfjsLib.getDocument({data:await f.arrayBuffer()}).promise,t='';for(let i=1;i<=doc.numPages;i++){let p=await doc.getPage(i),tc=await p.getTextContent();t+=tc.items.map(x=>x.str).join('\n')+'\n'}$('txt').value=t;pprev=parseLiquid(t,f.name);showP(pprev);$('pdfMsg').textContent='PDF leído. Si coincide con la liquidación real, guarda y reemplazará la fila Excel del mismo mes/tipo.'}catch(e){$('pdfMsg').textContent='No pude leer PDF: '+(e.message||e)+'. Usa Excel o pega texto.'}}
...
function savePdfPrev(){let d=$('pDate').value,uf=dec($('pUf').value),net=clp($('pNet').value);upsertRecord({...pprev,id:'r'+Date.now(),paymentDate:d,paymentYear:y(d),type:$('pType').value,managementYear:clp($('pMgmt').value)||y(d),description:$('pDesc').value,netClp:net,taxableBaseClp:clp($('pBase').value),withheldTaxClp:clp($('pTax').value),ufValue:uf,netUf:uf?net/uf:0,isProjected:false});$('prevP').classList.add('hide');save();nav('sDash')}
...
async function importJson(){try{let j=JSON.parse(await $('json').files[0].text());S={records:j.records||[],pensions:j.pensions||[]};save();$('bMsg').textContent='Importado.'}catch(e){$('bMsg').textContent='No pude importar JSON.'}}
